//sets are same as multiset with the exception that multiple elements can have the same value
...
int main()
{
	...
	multiset <int, greater <int> > gquiz1; //greater<int> the elements will be stored in descending order
    //if this ooption is not set the order of set will be ascending	 
...
// with value 30 in gquiz2
	...
	gquiz2.erase(gquiz2.begin()//erase function note this
				, gquiz2.find(30));
	for (itr = gquiz2.begin(); itr 
		!= gquiz2.end(); ++itr)
	{
