...
int number_of_nodes(Node *root,int data)//we can implement one of the traversal technique to count nodes
{
    if(root==NULL)
    return data;
    data++;
    data=number_of_nodes(root->first_child,data++);
    data=number_of_nodes(root->next_sibling,data++);
    return data;
}
int main()
{
    ...
    cout<<"The number of nodes in the given tree is: "<<number_of_nodes(root,0);
    return 0;
}