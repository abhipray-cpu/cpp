...
void printBottom_util(Node* root, int dist, int level, map<int,pair<int,int>> &map)
{
 if(root==NULL)
 return;
    ...
    if (level >= map[dist].second)
    {
     ...
     printBottom_util(root->left,dist-1,level+1,map);
     printBottom_util(root->right,dist+1,level+1,map);
}
void printBottom(Node *root)
{
map<int,pair<int,int>>map;
printBottom_util(root,0,0,map);
