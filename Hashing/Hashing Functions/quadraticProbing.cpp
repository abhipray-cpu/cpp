...
void LinearProbing::deleteValue(int value)
 {  bool found = false;
   ...
   {
      int hashValue=((value*value)/1000)+i*i;//since we are using quadratic probing though I should have kept the function simple
...
void LinearProbing::hashSearch(int value)
{
    ...
    for(int i=0;i<1000;i++) //will be adding the probing factor until 1) value is found size limit is reached
   {
      int hashValue=((value*value)/1000)+i*i;
       if(HashTable[hashValue] == value)
       {
           cout<<"Value found  \n";
           found = true;
...
{
    int hashValue =((value*value)/1000)+ProbingFactor*ProbingFactor;
